...
static ssize_t m74hc595_READ(struct file *filp,char __user *buf,size_t size,loff_t *loffp)
{
    ...
    gpio_set_value(m74hc595_dev.css_gpio,0);

    spi_read(spi, data, 2);
    copy_to_user(buf,data,2);

    gpio_set_value(m74hc595_dev.css_gpio,1);
    return 0;
}
...
static int m74hc595_probe(struct spi_device *spi)
{
    ...
    m74hc595_dev.nd = of_get_parent(spi->dev.of_node);
    if(IS_ERR(m74hc595_dev.nd)){
        pr_info("get parent node fail\n");
        ret = -EINVAL;
        return ret;
    }
    m74hc595_dev.css_gpio = of_get_named_gpio(m74hc595_dev.nd,"cs-gpios",0);
    if(m74hc595_dev.css_gpio<0){
        pr_info("get cs-gpio fail\n");
        ret = -EINVAL;
        return ret;
    }

    gpio_free(m74hc595_dev.css_gpio);
    ret = gpio_request(m74hc595_dev.css_gpio,DEVICE_NAME);
    if(ret<0){
    ...
    ret = gpio_direction_output(m74hc595_dev.css_gpio,1);
    if(ret<0){
        pr_info("gpio set mode fail\n");
        gpio_free(m74hc595_dev.css_gpio);
        ret = -EINVAL;
        return ret;
    }

...
static ssize_t m74hc595_READ(struct file *filp,char __user *buf,size_t size,loff_t *loffp)
{
    ...
    regmap_bulk_read(m74hc595_dev.regmap,0,data,2);
...
{   
    ...
    for(i=0;i<4;i++){
        value[0] = reg[i];
        value[1] = data[i];
        regmap_bulk_write(m74hc595_dev.regmap,0,value,2);
    }
...
static int m74hc595_probe(struct spi_device *spi)
{
    ...
    /*regmap init*/
    m74hc595_dev.config.reg_bits = 8;
    m74hc595_dev.config.val_bits = 8;

    m74hc595_dev.regmap = regmap_init_spi(spi,&m74hc595_dev.config);
    if(m74hc595_dev.regmap == NULL){
        pr_info("regmap init fail\n");
...
static int m74hc595_remove(struct spi_device *spi)
{
    ...
    regmap_exit(m74hc595_dev.regmap);
    return 0;
}
